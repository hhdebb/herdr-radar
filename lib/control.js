...
// live owner and the file is reclaimed. One endpoint per Herdr socket: the
// bind is the instance lock OF THAT SOCKET, which is what lets a named
// session's animator exist beside the default one instead of exiting at
// listen.
//
...
const { runtimeDir, socketId } = require('./paths');
const { NAME } = require('./identity');

function endpoint() {
  if (process.platform === 'win32') {
    // Per-user and per-socket so two accounts — or two sessions — on one
    // machine never fight over the name.
    return `\\\\.\\pipe\\${NAME}.${os.userInfo().username}.${socketId()}.ctl`;
  }
  const local = path.join(runtimeDir(), 'control.sock');
  // A unix socket path lives in sun_path: 108 bytes INCLUDING the terminator.
  // A state root nested deep enough pushes the bind past it and the daemon
  // exits at listen, so long layouts fall back to a short name in the system
  // temp dir that still carries the socket id.
  if (Buffer.byteLength(local, 'utf8') >= 108) {
    return path.join(os.tmpdir(), `${NAME}-${os.userInfo().username}-${socketId()}.sock`);
  }
  return local;
}
