...
const { stateRoot, ensureDir, isDefaultSocket } = require('./paths');
...
//
// One global block, one file: Herdr's tab-bar block cats the flat
// <stateRoot>/tabbar.txt, so only the default socket's daemon publishes. A
// named session writing here would let the last focused session paint over
// the others; its own line waits for the block to learn sessions.
function publish(line) {
  if (!isDefaultSocket()) return;
  const text = `${line}\n`;
