...
const { logoFor, nameFor, stateGlyph, throbFrame, blockedFrame } = require('./logos');
...
//
//   codex    quota
//   agy      ~/PINKPIXEL/PROJECTS/CURRENT/quota: agy - agy
//
...
// Does `text` name this pane's directory and nothing else? The shell
// abbreviates home to `~` and Herdr does not, so both spellings count, and so
// does the bare directory name — that is the whole of codex's title.
function namesDirectory(text, cwd, allowBasename) {
  const home = os.homedir();
  const short = home && cwd.startsWith(home + path.sep) ? `~${cwd.slice(home.length)}` : cwd;
  return text === cwd || text === short || (allowBasename && text === path.basename(cwd));
}
