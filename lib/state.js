...
const { stateRoot, runtimeDir, isDefaultSocket } = require('./paths');
...
// Signal files live one directory per Herdr socket, not flat in stateRoot:
// every session on a named socket runs its own animator, and a second
// animator that reads the first one's lock never starts. The daemon watches
// its own runtime directory for these; temp-cleaning tools that delete a
// watched directory kill the watcher silently, so they stay under stateRoot,
// which is ours and nobody sweeps it. The flat animator.pid of versions up to
// 1.3.7 is the one exception: the old daemon knew only that file, so on the
// default socket a live legacy pid still counts as running — otherwise the
// upgrade starts a twin next to it. Named sockets were never served by that
// daemon and ignore the flat files entirely.
const LOCK = () => path.join(runtimeDir(), 'animator.pid');
const STOP = () => path.join(runtimeDir(), 'animator.stop');

// Pids the flat layout left behind. Only ever read on the default socket, and
// only to keep a still-running old daemon from gaining a twin.
const LEGACY_LOCK = () => path.join(stateRoot, 'animator.pid');
const LEGACY_STOP = () => path.join(stateRoot, 'animator.stop');
const LEGACY_SOCK = () => path.join(stateRoot, 'control.sock');
...
function animatorRunning() {
  try {
    if (pidAlive(Number.parseInt(fs.readFileSync(LOCK(), 'utf8').trim(), 10))) return true;
  } catch {
    // No readable lock of our own — the legacy file below may still speak.
  }
  if (!isDefaultSocket()) return false;
  let legacyAlive = false;
  try {
    legacyAlive = pidAlive(Number.parseInt(fs.readFileSync(LEGACY_LOCK(), 'utf8').trim(), 10));
  } catch {
    legacyAlive = false;
  }
  if (legacyAlive) return true;
  // A dead legacy lock must not block the next start: sweep the flat files
  // the old layout left behind, once, and report stopped.
  for (const file of [LEGACY_LOCK(), LEGACY_STOP(), LEGACY_SOCK()]) {
    try {
      fs.rmSync(file, { force: true });
    } catch {
      // A leftover only delays the next start by one wake.
    }
  }
  return false;
}
