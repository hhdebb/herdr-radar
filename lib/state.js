    ...
    return [
      {
        ...
        // last word on what the row shows. Mirror panes (herdr-mirror
        // streamers) carry no terminal title of their own, so fall back to
        // the `title` metadata slot (then the raw title) or mirrored rows
        // render blank.
        title: hook.apply(
          'title',
          vendorTitle(
            a.agent ?? '',
            stripVendorPulse(a.terminal_title_stripped ?? a.title ?? a.terminal_title ?? ''),
