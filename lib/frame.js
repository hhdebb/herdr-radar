...
class Frame {
  ...
  async render(now) {
    ...
    for (const entry of entries) {
      ...
      const indent = grouped
        ? heads.has(entry.pane)
          ? keys.nested(entry.workspace)
            ? state.INDENT
            : ''
          : keys.nested(entry.workspace)
            ? state.CHILD_INDENT
      ...
      this.paneJobs(entry, display, { tabs, keys, indent, spinStep, header }, now, deadlines, jobs);
    }
  ...
  sortKeys(entries, parents, worktrees) {
    ...
    return { minuteKey, wsKeys, parentOf, orphanRepo, nested };
  }
  ...
  displayOrder(entries, viewMode, { wsKeys, minuteKey }) {
  ...
  paneJobs(entry, display, { tabs, keys, indent, spinStep, header = '' }, now, deadlines, jobs) {
    const pane = entry.pane;
    const src = this.src;
    ...
    const line = state.composeLine(entry, display, config.showTab ? tabs.get(entry.tab) ?? '' : '', indent, spinStep);
    ...
    const key = line ? `${display}:${line.mark}:${line.logo}:${line.titlePrefix}:${title}` : '';
    if (this.lastLine.get(pane) !== key && this.writable(`line:${pane}`, now)) {
      const write = line ? state.writeState(src, pane, display, line, title) : state.clearState(src, pane);
      jobs.push(
        write.then((ok) => {
    ...
    if (this.lastSort.get(pane) !== sortPair && this.writable(`sort:${pane}`, now)) {
      jobs.push(
        herdr.reportMetadataAsync(pane, src, { sort_key: sortKey, ws_key: wsKey }).then((ok) => {
    ...
    // at its expiry. Until-seen badges are static — an event wakes us to
    // clear those.
  ...
  async groupJobs(entries, displayEntries, viewMode, grouped, wsAgents, workspaces, keys, now, deadlines) {
    ...
    const fingerprint =
      ...
      displayEntries.map((e) => `${e.workspace}/${e.pane}`).join(',');
