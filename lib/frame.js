...
class Frame {
      ...
      const indent = grouped
        ? heads.has(entry.pane)
          ...
          : keys.nested(entry.workspace)
            ...
            : state.INDENT
        : '';
      this.paneJobs(entry, display, { tabs, keys, indent, spinStep }, now, deadlines, jobs);
    }
  ...
  paneJobs(entry, display, { tabs, keys, indent, spinStep }, now, deadlines, jobs) {
    const pane = entry.pane;
    const src = this.src;
    ...
    const key = line ? `${display}:${line.mark}:${line.logo}:${line.titlePrefix}:${entry.title}` : '';
    if (this.lastLine.get(pane) !== key && this.writable(`line:${pane}`, now)) {
      const write = line ? state.writeState(src, pane, display, line, entry.title) : state.clearState(src, pane);
      jobs.push(
        write.then((ok) => {
