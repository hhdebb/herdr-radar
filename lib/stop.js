...
const { stateRoot, isDefaultSocket } = require('./paths');
...
async function stopAnimator({ purge = false } = {}) {
  ...
  if (reply?.ok) {
    ...
    // pane every frame. A daemon from before per-socket runtime only watches
    // the flat state root, so on the default socket the marker goes there too
    // or the old daemon never sees it; a named socket was never its watch, and
    // a flat marker there would stop a daemon that serves another session.
    try {
      fs.writeFileSync(state.STOP(), '', 'utf8');
      if (isDefaultSocket()) fs.writeFileSync(path.join(stateRoot, 'animator.stop'), '', 'utf8');
    } catch {
      // Nothing to signal.
    }
    ...
    if (!state.animatorRunning()) {
      try {
        ...
        // The flat copies the legacy path above may have written.
        if (isDefaultSocket()) {
          fs.rmSync(path.join(stateRoot, 'animator.pid'), { force: true });
          fs.rmSync(path.join(stateRoot, 'animator.stop'), { force: true });
        }
      } catch {
