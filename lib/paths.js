...
//
// Two halves: files that describe the machine (the setup stamp, the managed
// config blocks, the tab-bar line Herdr's global block cats out) live flat in
// stateRoot and are written once; files a RUNNING animator owns (its lock, its
// control socket, its view and activity state) live one level down, in
// stateRoot/sockets/<id> — one directory per Herdr socket. Every session on a
// named socket starts its own animator, and two animators sharing one lock
// file means the second one never starts.
...
// The Herdr socket this process talks to — the one thing that tells a default
// session apart from a named one. Every socket consumer (lib/ipc.js,
// lib/subscribe.js) reads it here so there is exactly one place to get it
// wrong.
function socketPath() {
  return path.resolve(process.env.HERDR_SOCKET_PATH ?? path.join(path.dirname(herdrConfigPath()), 'herdr.sock'));
}

// Ten hex characters of the socket path's sha256: short enough for a
// directory name and a pipe name, stable across processes for the same path.
function socketId() {
  return crypto.createHash('sha256').update(socketPath()).digest('hex').slice(0, 10);
}

// Where this socket's animator keeps its runtime files. Created on first use,
// so merely asking never fails a render.
function runtimeDir() {
  return ensureDir(path.join(stateRoot, 'sockets', socketId()));
}

// Whether this process belongs to the default session. Only there do the
// flat files of versions up to 1.3.7 still speak: the old daemon served that
// socket alone, and a named session was never its business.
function isDefaultSocket() {
  return socketPath() === path.resolve(path.join(path.dirname(herdrConfigPath()), 'herdr.sock'));
}
