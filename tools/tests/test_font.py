...
def fit_for(fits, name):
    """Per-glyph scale from codepoints.toml, matching tools/build_font.py."""
...
class TestFontStructure(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with (ROOT / "tools" / "codepoints.toml").open("rb") as config_file:
            cls.fits = tomllib.load(config_file).get("fit", {})
    ...
    def test_glyph_bounds_fill_cell(self):
        ...
        for name in self.font.getGlyphOrder():
            ...
            # without being stretched. `[fit]` scales the box per glyph, so the
            # bounds move with it rather than sitting at fixed numbers.
            fit = fit_for(self.fits, name)
            self.assertGreater(
                max(width, height), 500 * fit, f"{name} too small: {width}x{height}"
            )
            self.assertLessEqual(
                width, MAX_WIDTH * fit + 8, f"{name} too wide: {width} (fit {fit})"
            )
            self.assertLessEqual(
                height, MAX_HEIGHT * fit + 8, f"{name} too tall: {height} (fit {fit})"
            )
    ...
    def test_aspect_ratio_matches_source_svg(self):
        ...
        for name in self.font.getGlyphOrder():
            ...
            SVGPath(str(ROOT / "tools" / "svg" / f"{name}.svg")).draw(svg_pen)
