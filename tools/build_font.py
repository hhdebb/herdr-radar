...
def svg_glyph(path: Path, fit: float = 1.0):
    ...
    dx = x_max - x_min
    dy = y_max - y_min
    scale = min(MAX_WIDTH / dx, MAX_HEIGHT / dy) * fit
...
def fit_for(fits: dict, name: str) -> float:
    """Per-glyph scale, falling back to `default` then to 1.0."""
    return float(fits.get(name, fits.get("default", 1.0)))


def plugin_id() -> str:
    """The plugin id from the manifest, so the font's identity follows a rename."""
    with (ROOT / "herdr-plugin.toml").open("rb") as manifest:
        return tomllib.load(manifest)["id"]


def build(output: Path) -> None:
    with (ROOT / "tools" / "codepoints.toml").open("rb") as config_file:
    ...
    for name in names:
        svg_path = ROOT / "tools" / "svg" / f"{name}.svg"
        if not svg_path.exists():
            print(f"WARN: missing {svg_path}, skipping", file=sys.stderr)
            continue
        try:
            glyphs[name] = svg_glyph(svg_path, fit_for(fits, name))
            print(f"OK {name}")
    ...
    builder.setupNameTable(
        {
            ...
            "familyName": family,
            "styleName": "Regular",
            "uniqueFontIdentifier": f"{plugin_id()}:{version}",
...
def selfcheck(path: Path, fits: dict | None = None) -> None:
    ...
    for name in font.getGlyphOrder():
        if name == ".notdef":
            continue
        glyph = glyf[name]
        # Outline bounds, not glyph.xMin/xMax: TrueType's stored box is the
        # CONTROL-POINT box, and off-curve points sit outside the curve they
        # bend. On a mark built almost entirely from curves that reads ~2%
        # larger than the shape actually drawn, which failed a mark that fits
        # the cell perfectly well. Both sides of the comparison below are now
        # true outlines.
        glyph_pen = BoundsPen(None)
        glyph.draw(glyph_pen, glyf)
        width = glyph_pen.bounds[2] - glyph_pen.bounds[0]
        height = glyph_pen.bounds[3] - glyph_pen.bounds[1]
        bounds_pen = BoundsPen(None)
        SVGPath(str(ROOT / "tools" / "svg" / f"{name}.svg")).draw(bounds_pen)
        ...
        fit = fit_for(fits, name)
        assert width <= MAX_WIDTH * fit + 8 and height <= MAX_HEIGHT * fit + 8, (
            f"{name}: {width}x{height} overflows "
            f"{MAX_WIDTH * fit:.0f}x{MAX_HEIGHT * fit:.0f}"
        )
        checked += 1
...
def main() -> int:
    ...
    with (ROOT / "tools" / "codepoints.toml").open("rb") as config_file:
        fits = tomllib.load(config_file).get("fit", {})
    build(args.output)
    selfcheck(args.output, fits)
    return 0
